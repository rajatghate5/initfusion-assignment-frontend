...
//         },
//         headers: {
//           Authorization: `Bearer ${getAuthToken()}`, // Ensure you include the Bearer token
//         },
//       });
...
//   async (newSchedule) => {
//     const response = await axios.post(`${API_URL}/api/v1/add-schedule`, newSchedule, {
//       headers: {
//         Authorization: `Bearer ${getAuthToken()}`,
//       },
//     });
...
//         state.schedules.push(action.payload); // Fixed from `state.schedules.schedules.push`
//       })
...
//         state.status = "succeeded";
//         // Optionally, you can store the fetched schedule in a separate state if needed
//         // Example: state.selectedSchedule = action.payload;
//       })
...
const scheduleSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addSchedule.fulfilled, (state, action) => {
        state.schedules.schedules.push(action.payload);
      })
      .addCase(updateSchedule.fulfilled, (state, action) => {
